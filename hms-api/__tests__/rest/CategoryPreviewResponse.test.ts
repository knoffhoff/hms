...
describe('Compare', () => {
  test('All values different', () => {
    const a = CategoryPreviewResponse.from(makeCategory(
        {title: 'a'} as CategoryData));
    const b = CategoryPreviewResponse.from(makeCategory(
        {title: 'b'} as CategoryData));
